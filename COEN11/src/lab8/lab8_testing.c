/***************************************************************************************************/
/*							    	Dylan Hoover COEN 11 Lab 8                                     */
/*									  8 November 2018							                   */
/*	Stuct of linked lists that output to a text file, autosave to a binary                         */
/***************************************************************************************************/
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
void readFile(char *file)
{
	...
	while(fscanf(fp, "%s %d\t", tempName, &tempDep) != EOF)
	{
		switch(tempDep)
		{
			case 1:
...
void displaySave(char *file)
{
	FILE  *bfp;
	bfp = fopen(file, "rb");
	NODE binary;
	printf("Names	Department	Issue\n");
	while(fread(&binary, sizeof(NODE), 1, bfp) == 1)
	{
		if(binary.department == 1)
				printf("%s	%d		%.2f degree fever\n",binary.name, binary.department,binary.additionalInfo.fever);
		if(binary.department == 2)
				printf("%s	%d		%s pain\n",binary.name,binary.department,binary.additionalInfo.pain);
		if(binary.department == 3)
				printf("%s	%d		%d days sick\n",binary.name,binary.department,binary.additionalInfo.daySick);
		if(binary.department == 4)
				printf("%s	%d		%d days sick\n",binary.name,binary.department,binary.additionalInfo.daySick);
